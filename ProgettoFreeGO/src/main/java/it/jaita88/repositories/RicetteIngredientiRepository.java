...
public interface RicetteIngredientiRepository extends CrudRepository<RicetteIngredienti, Integer> {
	...
	@Query(value="SELECT ricette.nome FROM ricette JOIN ricette_ingredienti ON ricette.id=ricette_ingredienti.id_ricetta JOIN ingredienti ON ricette_ingredienti.id_ingrediente=ingredienti.id WHERE ingredienti.nome=?1", nativeQuery = true)
